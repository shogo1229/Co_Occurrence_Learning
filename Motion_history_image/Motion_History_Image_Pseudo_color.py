...
class MotionHistoryImage_Pseudo():
    ...
    def create_Pseudo_color_MHI(self,MHI,frameDifferenceImage,motionHistoryImage):
        ...
        Pseudo_color_motionHistoryImage = cv2.applyColorMap(motionHistoryImage, cv2.COLORMAP_HOT)       #疑似カラーを付与
        #Pseudo_color_motionHistoryImage[np.where((Pseudo_color_motionHistoryImage == [255,0,0]).all(axis=2))] = [0,0,0]
        return Pseudo_color_motionHistoryImage,MHI

    def __call__(self,MoviePath = 0 ,SaveFlag = False , DisplayFlag = False ,MHI_SavePath = None ,RGB_SavePath = None,CoOccurrence_SavePath = None,Pseudo_color = True):
        ...
        while(capture.isOpened()):
            ...
            if ret == True:
                ...
                Co = cv2.addWeighted(src1=frameImage,alpha=1,src2=motionHistoryImage,beta=0.7,gamma=0)
                if DisplayFlag == True:

...
from Dataloder.ver2_Pseudo_MotionHistory_dataloder import PseudoMotionHistory_Dataset
...
if __name__ == '__main__':
    ...
    Traindataset = PseudoMotionHistory_Dataset(TrainPath, transform, 5)
    Testdataset = PseudoMotionHistory_Dataset(TrainPath, transform, 5)
    print("Load TrainDataSet 「"+str(TrainPath)+"」")
    print("Load TestDataSet 「"+str(TestPath)+"」")
    ...
    trainLoader = torch.utils.data.DataLoader(Traindataset, batch_size=BatchSize, shuffle=True, num_workers=os.cpu_count(), pin_memory=True)
    valLoader = torch.utils.data.DataLoader(Testdataset, batch_size=BatchSize, shuffle=False, num_workers=os.cpu_count(), pin_memory=True)

...
Fold = 10               
Accuracy = [0]*Fold                               
DatasetPath = r"C:\Users\DeepLearning\Documents\Wild-Life_4th\MHI"                                           
modelPath = r"max_Val_acc_MobileNet_MHI_Wild-Life4th_Kfold_part1"        
...
class Trainer():
    ...
    def Train(self,epoch):
        ...
        for batchIdx,(img,label) in enumerate(self.trainLoader):
            ...
            train_log = "epoch : {:3} train_loss : {:3.8} train_acc : {:3.8}".format(str(epoch+1), str(t_loss), str(t_acc))
        ...
        with torch.no_grad():
            for batchIdx,(img,label) in enumerate(self.valLoader):
                ...
                val_log = train_log + " val_loss : {:3.8} val_acc : {:3.8}".format(str(v_loss), str(v_acc))
                print("\r"+val_log,end="")
        ...
        if v_acc > self.max_acc:
            self.max_acc = v_acc
            with open("models/"+str(modelPath)+"/Fold-"+str(self.fold)+"_"+str(modelPath)+".pth", "wb") as f:
...
if __name__ == '__main__':
    ...
    dataset = PseudoMotionHistory_Dataset(DatasetPath, transform, 5)

...
class MotionHistoryImage_prev5():
    ...
    def __call__(self,MoviePath = None ,SaveFlag = False , DisplayFlag = False ,MHI_SavePath = None ,RGB_SavePath = None,CoOccurrence_SavePath = None):
        ...
        ret,prevImage4 = capture.read()                                                                 #フレーム読み込み
        MHI = np.zeros((prevImage4.shape[0],prevImage4.shape[1]), np.uint8)
        motionHistoryImage = np.zeros((prevImage4.shape[0], prevImage4.shape[1], 1), np.uint8)

        prevImage4= cv2.cvtColor(prevImage4,cv2.COLOR_BGR2GRAY)   
        ret,frameImage = capture.read()   
        prevImage3= cv2.cvtColor(frameImage,cv2.COLOR_BGR2GRAY)                                      #frameImageをグレースケール化し,t-4の画像とする
        ret,frameImage = capture.read()                                                                 #フレーム読み込み
        prevImage2 = cv2.cvtColor(frameImage,cv2.COLOR_BGR2GRAY)                                     #frameImageをグレースケール化し,t-3の画像とする
        ret,frameImage = capture.read()                                                                 #フレーム読み込み
        ...
        while(capture.isOpened()):
            ...
            if ret == True:
                ...
                if ret == False:    
                    break        
                currentImage = cv2.cvtColor(frameImage,cv2.COLOR_BGR2GRAY)                                 #frameImageをグレースケール化し,t+1の画像とする                                                 
                differenceImage1 = cv2.absdiff(prevImage3,prevImage4)                                    #(t-3)-(t-4)の差分
                differenceImage2 = cv2.absdiff(prevImage3,prevImage2)                                  #(t-3)-(t-2)の差分
                differenceImage3 = cv2.absdiff(prevImage2,prevImage)                                  #(t-2)-(t-1)の差分
                differenceImage4 = cv2.absdiff(prevImage,currentImage)                                    #(t-1)-(t)の差分
                ...
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
                prevImage4 = prevImage3.copy()
                prevImage3 = prevImage2.copy()
                prevImage2 = prevImage.copy()

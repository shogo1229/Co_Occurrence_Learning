...
def resnet18_temporal(pretrained=False, channel= 15, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], nb_classes=11, channel=channel, **kwargs)
    if pretrained:
...
def resnet34_temporal(pretrained=False, channel= 20, **kwargs):

    model = ResNet(BasicBlock, [3, 4, 6, 3], nb_classes=3, channel=channel, **kwargs)
    if pretrained:
...
def resnet50_temporal(pretrained=False, channel= 15, **kwargs):

    model = ResNet(Bottleneck, [3, 4, 6, 3], nb_classes=51, channel=channel, **kwargs)
    if pretrained:
...
def resnet101_temporal(pretrained=False, channel= 20, **kwargs):

    model = ResNet(Bottleneck, [3, 4, 23, 3],nb_classes=51, channel=channel, **kwargs)
    if pretrained:
...
def resnet152_temporal(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    model = resnet34_temporal(pretrained= True, channel=20)
    print (model)

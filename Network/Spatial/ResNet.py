...
def resnet18_spatial(pretrained=False, channel= 3, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], nb_classes=6, channel=channel, **kwargs)
    if pretrained:
...
def resnet34_spatial(pretrained=False, channel= 20, **kwargs):
...
def resnet50_spatial(pretrained=False, channel= 3, **kwargs):
...
def Resnet101_spatial(pretrained=False, channel= 20, **kwargs):

    model = ResNet(Bottleneck, [3, 4, 23, 3],nb_classes=3, channel=channel, **kwargs)
    if pretrained:
...
def resnet152_spatial(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    model = resnet34_spatial(pretrained= True, channel=20)
    print (model)

...
class MotionHistoryImage_Pseudo():
    ...
    def create_Pseudo_color_MHI(self,MHI,frameDifferenceImage,motionHistoryImage):
        idx_PIXEL_MAX_VALUE = np.where(frameDifferenceImage == self.PIXEL_MAX_VALUE)                    #更新された部分のインデックスを取得(返り値はタプル)
        MHI[idx_PIXEL_MAX_VALUE] = self.TAU                                                             #更新された部分の値にTAU(残すフレーム数)を代入
        idx_TAU = np.where(MHI > self.PIXEL_MIN_VALUE)                                                  #画素値が1以上の部分のインデックスを取得
        MHI[idx_TAU] = MHI[idx_TAU] - 1                                                                 #画素値が1以上の全て画素値に対して-1
        ...
        motionHistoryImage = np.fix(motionHistoryImage)                                                 #一個上でfloat型になったから整数に
        motionHistoryImage = motionHistoryImage.astype(np.uint8)                                        #floatで返すと0,255の値で返されるからunit8型に変換
        Pseudo_color_motionHistoryImage = cv2.applyColorMap(motionHistoryImage, cv2.COLORMAP_RAINBOW)       #疑似カラーを付与
        Pseudo_color_motionHistoryImage[np.where((Pseudo_color_motionHistoryImage == [0,0,255]).all(axis=2))] = [0,0,0]
        return Pseudo_color_motionHistoryImage,MHI

    def __call__(self,ImageList = None ,SaveFlag = False , DisplayFlag = False ,MHI_SavePath = None ,RGB_SavePath = None,CoOcc_SavePath = None,Pseudo_color = False):
        for i,image in enumerate(ImageList):
            ...
            motionHistoryImage,MHI = self.create_Pseudo_color_MHI(MHI,frameDifferenceImage,motionHistoryImage)
            nextImage2 = cv2.cvtColor(motionHistoryImage,cv2.COLOR_BGR2GRAY)  
            ...
            if SaveFlag == True: 
                ...
                cv2.imwrite(str(CoOcc_SavePath) +'/CoOcc_'+ str(self.count) + ".jpg",CoOccurrence_Image )
                #cv2.imwrite(str(RGB_SavePath) +'/RGB_'+ str(self.count) + ".jpg",frameImage )
                if self.count % self.TAU == 0:
                    #cv2.imwrite(str(RGB_split_SavePath) +'/RGB_split_'+ str(self.count) + ".jpg",frameImage )
                    None
